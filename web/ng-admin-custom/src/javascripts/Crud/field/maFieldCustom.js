export default function maFieldCustom(FieldViewConfiguration, $compile) {
    return {
        restrict: 'E',
        scope: {
            field: '&',
        ...
        link: function(scope, element) {
            ...
            scope.getCssClasses = function(entry) {
                return 'ng-admin-field-' + field.name().replace('.', '_') + ' ng-admin-type-' + type + ' ' + (field.getCssClasses(entry) || '');
            };
...
`<div ng-class="getCssClasses(entry)">
    ...
    <span ng-show="fieldHasValidation()" class="glyphicon form-control-feedback" ng-class="fieldIsValid() ? 'glyphicon-ok' : 'glyphicon-remove'"></span>
                ...
                fieldTemplate =
`<div ng-class="field.getCssClasses(entry)||'col-sm-12'">
...
maFieldCustom.$inject = ['FieldViewConfiguration', '$compile'];
